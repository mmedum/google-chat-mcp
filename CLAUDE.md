...
- FastMCP `~= 3.2` (current 3.4.7; 3.4.0 migrated auth JWTs to `joserfc`, and
  `fastmcp` is now a shim over `fastmcp-slim`)
- `ty == 0.0.73` (pinned exactly — it's 0.0.x beta, every patch can have
...
- `ruff ~= 0.15` (current 0.16.3; 0.16 stabilised `PLR0917`, hence its entry in
  the ignore list)
