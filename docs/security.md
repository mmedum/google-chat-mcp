...
10. **stdio scope check is pre-flight** — `granted_scopes` from tokens.json compared against `required_scope` before the upstream API call; an unknown or unparseable set defers to Google's 403 rather than rejecting locally, and a denial is audited either way. Defence in depth only — Google enforces scopes regardless. (This check never fired before the fix in the Unreleased CHANGELOG entry: the stored value was the *requested* set.)
