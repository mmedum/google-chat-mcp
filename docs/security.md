...
10. **stdio scope check is pre-flight** — `granted_scopes` from tokens.json checked against every scope the tool accepts (`required_scope` plus any `also_accepts`) before the upstream API call, each via `scope_satisfied`, so an umbrella scope satisfies anything split out of it — matching what Google accepts. An unknown or unparseable set defers to Google's 403 rather than rejecting locally, and a denial is audited either way. Defence in depth only — Google enforces scopes regardless.
