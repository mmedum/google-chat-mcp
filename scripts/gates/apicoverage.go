...
//     API this server can reach, with its verb and path, as Google
//     published them on the day it was fetched. `gates api-diff` writes
//     it. Nobody edits it by hand.
//   - testdata/api-coverage.tsv is the record: one verdict per method.
//     `used` names the gchat.Client method that implements it, `out`
//     gives the reason it is deliberately not called.
//
...
//
// What none of this checks is which method a row is about. A `used` row
// is held to naming an existing, unclaimed client method, and the
// snapshot is held to what Google publishes, but nothing binds the two:
// swap the reasons on `users.availability.markAsActive` and `markAsAway`
// and every gate stays green over a record that names the wrong method
// twice. Closing it means reading the URL each method builds, which is a
// request literal followed through three fields and a shared helper —
// more machinery than the mistake is worth. It is written down instead.
//
// Also not derived: the reason on an `out` row. Sixteen of them say a
// method needs a scope this server does not ask for, and Google
// publishes each method's scopes, so those verdicts are a join of two
// lists that already exist. Adding a scope to `scopes.All` would make
// those reasons wrong with nothing failing. Deriving them would tie the
// record to a scope-set intersection for sixteen rows that change when
// somebody deliberately widens what this server asks for — a change that
// would visit these rows anyway. Left as prose, on purpose.
const (
...
// The two verdicts a row may carry.
const (
	verdictUsed = "used"
	verdictOut  = "out"
)
...
func apiCoverage(_ []string, stdout, stderr io.Writer) int {
	...
	_, _ = fmt.Fprintf(stdout, "api coverage ok (%d methods across %d APIs: %d used, %d deliberately out; "+
		"snapshot fetched %s)\n", len(entries), len(apis), used, len(entries)-used, snap.Fetched)
	return 0
}
...
func readCoverage(path string) ([]coverageEntry, []string) {
	...
	for n := 1; scanner.Scan(); n++ {
		...
		seen[e.key] = n
		switch e.verdict {
		case verdictUsed, verdictOut:
		default:
			fail(n, "%s has verdict %q; want %s or %s", e.key, e.verdict, verdictUsed, verdictOut)
		}
...
func collectMethods(prefix string, res resource, out map[string]apiMethod) {
	for name, m := range res.Methods {
		out[prefix+name] = apiMethod{Verb: m.HTTPMethod, Path: m.Path}
	}
